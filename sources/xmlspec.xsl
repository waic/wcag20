...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://icl.com/saxon" exclude-result-prefixes="saxon" version="1.0">
	...
	<!-- xmlspec.xsl: An HTML XSL[1] Stylesheet for XML Spec V2.1[2] markup
     ...
     Version: $Id: xmlspec.xsl,v 1.63 2010/02/04 00:59:57 cooper Exp $
     ...
     Date:    Created 07 September 1999
              Last updated $Date: 2010/02/04 00:59:57 $ by $Author: cooper $
  ...
  -->
	...
	tbody tfoot thead tr trdisclaimer trnote   typedef ul ulist union vc vcnote wfc wfcnote issue"/>
...
-->
	...
	<xsl:template match="latestloc">
		<xsl:choose>
			...
			<xsl:otherwise>
				<dt xmlns="http://www.w3.org/1999/xhtml">最新バージョン:</dt>
				<dd xmlns="http://www.w3.org/1999/xhtml">
					<xsl:apply-templates/>
				</dd>
			</xsl:otherwise>
  ...
  -->
	...
	<xsl:choose>
		<xsl:when test="@role='cite'">
			<cite xmlns="http://www.w3.org/1999/xhtml"><xsl:apply-templates></xsl:apply-templates></cite>
		</xsl:when>
		<xsl:otherwise>
	...
	<xsl:template match="publoc">
		<dt xmlns="http://www.w3.org/1999/xhtml">このバージョン:</dt>
		<dd xmlns="http://www.w3.org/1999/xhtml">
			<xsl:apply-templates/>
		</dd>
	</xsl:template>
	...
	<xsl:template match="termdef">
		<xsl:text></xsl:text>
		<a xmlns="http://www.w3.org/1999/xhtml" name="{@id}" id="{@id}" title="{@term}">	</a>
		 <strong xmlns="http://www.w3.org/1999/xhtml"><xsl:value-of select="@term"></xsl:value-of>:</strong>
		<xsl:text> </xsl:text>
		<xsl:apply-templates/>
		<xsl:text> </xsl:text>
	</xsl:template>
  ...
  -->
	...
	<link xmlns="http://www.w3.org/1999/xhtml" media="print" type="text/css" rel="stylesheet" href="print.css" />
