	...
	<xsl:template match="procedure">
		...
		<xsl:variable name="id">
			...
			<xsl:text>-procedure</xsl:text>
					...
					<xsl:choose>
      <xsl:when test="$slices=1"><h3 class="small-head" id="{$id}">手順</h3></xsl:when>
	<xsl:when test="$bytech=1"><h4 class="small-head" id="{$id}">手順</h4></xsl:when>
      <xsl:otherwise><h5 class="small-head" id="{$id}">手順</h5></xsl:otherwise>
    </xsl:choose>
			    ...
			    <xsl:when test="child::node()">
			      <xsl:apply-templates/>
			    	...
			    	<xsl:if test="not(ancestor::div1[@id = 'failures'])">
			    		<p>この達成方法が「十分な達成方法」の一つである場合、この手順や判定基準を満たしていなければ、それはこの達成方法が正しく用いられていないことを意味するが、必ずしも達成基準を満たしていないことにはならない。場合によっては、別の達成方法によってその達成基準が満たされていることもありうる。</p>
			    	</xsl:if>
			    </xsl:when>
...
<xsl:apply-templates select="issue"/>			
    ...
    <xsl:otherwise><h4 class="small-head" id="{$uanumber}">ユーザエージェント及び支援技術によるサポート</h4></xsl:otherwise>
			...
			<p><!--a href="https://www.w3.org/WAI/WCAG20/Techniques/ua-notes/{ancestor::div1/@id}#{ancestor::technique/@id}"--><a href="./ua-notes/{ancestor::div1/@id}.html#{ancestor::technique/@id}"><xsl:value-of select="ancestor::technique/@id"/> に関するユーザエージェントサポートノート</a>を参照のこと。<xsl:if test="ancestor::div1/div2"><a><xsl:attribute name="href"><xsl:call-template name="href.target"><xsl:with-param name="target" select="ancestor::div1/div2[1]"/></xsl:call-template></xsl:attribute><xsl:value-of select="ancestor::div1/div2[1]/head"/></a>も参照。</xsl:if></p>
		</div>
	</xsl:template>
